...
14. 키가 존재하면 Car Pay-in 사전 등록 차량이므로 키를 삭제(consume)하고 Car Pay-in Backend에 `POST /webhook/entry`로 입차 이벤트를 보낸다. 키가 없으면 webhook을 보내지 않는다.
15. 백엔드는 carpayin-redis의 `parking_pre_notify:{lot_id}:{plate}`를 조회해 `car_id`를 가져온다.
16. 백엔드는 Car Pay-in DB의 `parking_sessions`에 서비스 기준 주차 세션을 생성한다.
17. 백엔드는 사용이 끝난 `parking_pre_notify:{lot_id}:{plate}`를 삭제한다.
18. 백엔드는 MQTT로 앱에 입차 알림을 전송한다.
