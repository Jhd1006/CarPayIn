...
13. PMS는 pms-redis에 `parking_session:{lot_id}:{plate}`를 `status=active`로 저장한다 (TTL 72시간). 출차 판단과 결제 상태 확인에 사용하는 실시간 상태판이다.
14. PMS는 pms-redis에서 `pre_reg:{lot_id}:{plate}` 키를 조회한다.
15. 키가 존재하면 Car Pay-in 사전 등록 차량이므로 키를 삭제(consume)한다. 키가 없으면 webhook을 보내지 않는다.
...
22. 백엔드는 MQTT 브로커를 통해 앱에 입차 알림을 전송한다.
    - 발행 성공 → 완료.
    - 발행 실패 → `entry_notify_retry:{session_id}`에 이벤트 저장 (TTL 1시간). `NotifyRetryWorker`가 60초마다 재시도.
...
- `parking_session:{lot_id}:{plate}`: LPR 입차 시 생성. `status=active` 상태. TTL 72시간. 이후 결제 완료 시 `paid`로 갱신되고, 출차 확인 후 삭제된다.
