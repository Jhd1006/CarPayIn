...
외부 시스템이 직접 호출하는 endpoint는 app access token 대신 공통 webhook signature header를 검증한다.
...
`X-Webhook-Timestamp`, `X-Webhook-Signature`가 보안 경계 역할을 하므로 API test에서 반드시 확인한다.
