...
class FakeParkingSessionRepository:
    ...
    def add_session(
        ...
        plate: str,
        entry_time: str,
        pms_session_id: str = "pms-sess-001",
        status: str = "active",
    ):
...
class TestGetParkingFee:
    ...
    def test_session_not_active_raises_error(
        ...
        """session이 active가 아니면 session_not_active 오류를 반환한다."""
    ...
    def test_session_car_id_mismatch_raises_error(
        ...
        """session 소유 차량과 token 차량이 불일치하면 session_car_id_mismatch 오류를 반환한다."""
        fake_parking_session_repository.add_session(
            session_id=VALID_SESSION_ID,
            car_id="other-car-id",
            lot_id=VALID_LOT_ID,
    ...
    def test_redis_quote_exists_does_not_call_pms(
        ...
        """Redis quote가 있으면 PMS를 호출하지 않고 quote를 그대로 반환한다."""
        fake_parking_session_repository.add_session(
            session_id=VALID_SESSION_ID,
            car_id=VALID_CAR_ID,
            lot_id=VALID_LOT_ID,
        ...
        fake_fee_quote_store.save_quote(
            ...
            lot_id=VALID_LOT_ID,
            amount=VALID_AMOUNT,
            duration=VALID_DURATION, #추가된 ITEM
            currency=VALID_CURRENCY,
            entry_time=VALID_ENTRY_TIME,
    ...
    def test_redis_quote_not_exists_calls_pms_and_saves_quote(
        ...
        assert saved_quote["duration"] == VALID_DURATION #추가된 item
        ...
        assert result.lot_id == VALID_LOT_ID
        assert result.amount == VALID_AMOUNT
        assert result.duration == VALID_DURATION #추가된 item
        assert result.currency == VALID_CURRENCY
        assert result.status == "active"