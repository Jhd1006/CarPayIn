...
class TestNotifyPmsPaymentComplete:
    ...
    def test_payment_success_generates_correct_payload(
        ...
        """결제 성공 후 PMS 통보 payload가 정확히 생성된다."""
        ...
        assert call["carpay_parking_session_id"] == VALID_CARPAY_SESSION_ID
        assert call["carpay_tx_id"] == VALID_TX_ID
    ...
    def test_transaction_not_success_raises_error(
        self,
        notify_pms_payment_complete_service,
        fake_transaction_repository,
        fake_pms_client,
    ):
        """transaction이 success 상태가 아니면 PMS를 호출하지 않고 오류를 반환한다."""
        fake_transaction_repository.add_transaction(VALID_TX_ID, status="failed")
        ...
        with pytest.raises(ValueError) as exc_info:
            notify_pms_payment_complete_service.execute(command)
 
        assert str(exc_info.value) == "transaction_not_success"
    ...
    def test_pms_timeout_preserves_payment_and_records_retry(
        ...
        """PMS timeout 시 결제 성공은 보존되고 재시도 이벤트가 기록된다."""
        fake_transaction_repository.add_transaction(VALID_TX_ID, status="success")
    ...
    def test_pms_5xx_preserves_payment_and_records_retry(
        ...
        """PMS 5xx 응답 시 결제 성공은 보존되고 재시도 이벤트가 기록된다."""
        fake_transaction_repository.add_transaction(VALID_TX_ID, status="success")
    ...
    def test_duplicate_idempotency_key_is_safe(
        ...
        """같은 idempotency_key로 PMS 통보가 중복되어도 안전하게 처리된다."""
        fake_transaction_repository.add_transaction(VALID_TX_ID, status="success")
